        ...
        <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : " "}`}>
            <div className='pb-5'>
                ...
                <div className="bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5">
                    ...
                    <input onChange={(e)=> setInput(e.target.value)} type="text" placeholder="serach User....."
                        ...
                        <div className="flex flex-col leading-5">
                            <p>{user.fullName}</p>
                            {
                               onlineUsers.includes(user._id)
                                    ...
                                    : <span className="text-neutral-400 text-ts ">offline</span>
                            }
                        </div>
                        {unseenMessages[user._id]>0&& <p className="absolute top-4 right-4 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-voilet-500/50">{unseenMessages[user._id]}</p>}
                    </div>
                ))}
            </div>
        </div>
