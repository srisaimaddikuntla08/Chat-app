...
function RightSidebar() {
  ...
  useEffect(()=>{
      setMsgImages(
        messages.filter(msg=>msg.img).map(msg=>msg.image)
      )
  },[messages])
  ...
  return selectedUser &&(
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll ${selectedUser?"mx-md:hidden":""}`}>
      ...
      bg-gradient-to-r from-purple-400 to voilet-600 text-white border-none text-sm font-light py-2 px-20 rounded-full cursor-full '>
        Logout
      </button>
    </div>
  )
